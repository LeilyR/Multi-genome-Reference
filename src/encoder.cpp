...
//		for(size_t k = 0; k < data.numAcc(); k++){
	...
	void encoder:: test_al_encoding(map<string, unsigned int> & weight, map<string, string > & cluster_members, map<string, vector<pw_alignment> > & alignmentOfCluster, ofstream &outs,dlib::entropy_encoder_kernel_1 & enc ){
...
//		size_t i = 1;
	...
	void encoder::test_al_decoding(ifstream & in, dlib::entropy_decoder_kernel_1 & dec ){
			...
			while(target < flag + 15){//end of an accession
				...
				while(target<flag+10){//end of each seq
					...
					if(flag<=target && target < flag+10){
						...
						//	cout << "cent_acc " << cent_acc << " other_acc " << i <<endl;
						...
						//	cout << "mod->second size: " << mod->second.size() << " target: "<< target<<endl;
						...
						//	cout<< "modification: " << modification << " low: " << al_low.at(modification) << " high: " <<  al_high.at(modification) <<endl;//problem is getting modification = 0 
...
//						}
						target = dec.get_target(total);	
						cout<< "problemistic target: "<< target << endl;
						reverse_center = 0;
						reverse_member = 0;
						reverse_both = 0;
					}
...
/*	void encoder::test_al_decoding(ifstream & in){
		...
		while(target < flag + 5){
			...
			map<string, string>::iterator seq = decoded_center_in_partition.at(0).find(center);
			string decodedCenter =seq->second;
			cout<< "decoded center: "<< decodedCenter <<endl;
			vector<string> center_parts;
			strsep(center, ":" , center_parts);
			unsigned int cent_ref = atoi(center_parts.at(0).c_str());
	...
	void encoder::arithmetic_encoding_alignment(map<string, unsigned int> & weight, map<string, string > & cluster_members, map<string, vector<pw_alignment> > & alignmentOfCluster, ofstream & outs, dlib::entropy_encoder_kernel_1 & enc){
		calculate_high_in_partition(weight,alignmentOfCluster);
		encoding_functor functor(data,&model,wrappers,enc);
		arithmetic_encoding_centers(alignmentOfCluster,outs,enc);
		size_t bit = 13;
		...
		for(size_t i = 2; i< data.numAcc(); i++){
			for(size_t k = 0; k <data.getAcc(i).size(); k++){//retruns all sequences of an acc
				...
				for(size_t n= 0; n < sequence.length(); n++){
					...
					if(it != AlignmentsFromClustering.at(sequenceId).end()){//If there is an alignment on that position
						...
						cout<< "we are on an al"<<endl;
						stringstream mem;
						mem << sequenceId << ":" << n;
						map<string, string>::iterator cl = cluster_members.find(mem.str());
						string center = cl->second;
						cout<< "encoding the center: "<< center << endl;
						vector<string> center_parts;
						strsep(center, ":" , center_parts);
						unsigned int cent_ref = atoi(center_parts.at(0).c_str());
						unsigned int cent_left = atoi(center_parts.at(1).c_str());
						...
						if(cent_ref == sequenceId && cent_left == n){ //when center itself is on the sequence
							...
							model.get_encoded_member(*p,cent_ref,cent_left,functor,outs);							
							cout<<"center is not on the sequence"<<endl;
						}
			...
			unsigned int h1 = model.get_powerOfTwo().at(bit) + 20;
			enc.encode(l1,h1,total);
			cout<<"end of a enc an accession" << endl;
			wrappers.encode(l1,h1,total); 
		}
	}
	void encoder::arithmetic_decoding_alignment(ifstream& in,dlib::entropy_decoder_kernel_1 & dec){
//		dlib::entropy_decoder_kernel_1  dec;
		arithmetic_decoding_centers(in,dec);
		cout<<"decoding the center has been done!"<<endl;
		size_t bit = 13;
					...
					/*	if(target>=flag+15&&target<flag+20){
						...
						while(target < flag+5){//decoding modifications:
							...
							for(size_t m =0; m < al_context.size(); m++){
								cout<< "al_context: "<< int(al_context.at(m));
							}
							cout<< " " <<endl;
							map<string, vector<unsigned int> >::const_iterator mod =model.get_highValue(cent_acc,i).find(al_context);//modifiactions are always from center to the other sample
							assert(mod != model.get_highValue(cent_acc,i).end());
