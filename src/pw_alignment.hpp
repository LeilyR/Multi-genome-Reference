...
class compare_pointer_alignment {
	...
	bool operator()(const alignment_type *ar, const alignment_type *br) const;
};

template<typename alignment>
class compare_alignment {
	public:
	typedef alignment alignment_type;
	bool operator()(const alignment_type &a, const alignment_type &b) const ;
};

class sort_pw_alignment_by_left{
	public:
	bool operator () (const pw_alignment &p1, const pw_alignment &p2 )const;
};
class sort_pw_alignment_by_right{
	public:
	bool operator () (const pw_alignment &p1, const pw_alignment &p2 )const;
};
...
class wrapper{
	public:
		wrapper();
		~wrapper();
		void encode(unsigned int&, unsigned int &, unsigned int&, std::ofstream &);
		void context(int , std::ofstream &);
				
	private:
		std::ofstream encode_boundries;
		std::ofstream encode_context;
