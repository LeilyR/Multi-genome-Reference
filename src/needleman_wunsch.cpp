...
//	}
	...
	template<typename T>
	void needleman<T>::compute_matrix(size_t & read_id, size_t & ref_id){//TODO add types later to deal with different ending cases.
	...
	void needleman<T>::find_the_best_path(size_t & type, std::string & from_read , std::string & from_ref){
		...
		size_t row,column;//TODO think of type four when both strat and end are not fixed
		if(type == 1 || type == 2){//when both start and end or only the end is fixed. We need to start from the last cell in the matrix.
			row = read.length();
			column = ref.length();
		}else{
			assert(type ==3); //when the started is fixed but end is not.
