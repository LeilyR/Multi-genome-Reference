...
class encoder{
	...
	void partitioning_long_centers(std::map<std::vector<std::string>, std::vector<pw_alignment> > &, std::map<std::string ,std::vector<pw_alignment> > &,std::map<std::vector<std::string>, unsigned int > &);// It includes bot long and original centers
	void calculate_high_in_partition(std::map<std::string, unsigned int> & , std::map<std::string,std::vector<pw_alignment> > & );
	void partition_high();
	void calculate_long_center_high(std::map<std::vector<std::string>, std::vector<pw_alignment> > &,  std::map<std::string ,std::vector<pw_alignment> > & , std::map<std::vector<std::string>, unsigned int> &);//It also includes both long and original ones
...
class decoder{
	...
	std::vector< std::vector<std::pair<size_t, vector<unsigned int> > > >cluster_high_partition;//vector ---> partitions , vector<pair> --> center of each partition with their high and low value.
	std::map<std::string,std::string> decoded_centers; // string --->dna sequence, string ---> ref:left(ino estefade nemikonam)
//	std::vector<std::string> centerId;//keep the centers in the order that has been decoded
	std::map<size_t , std::vector<std::string> > partition; // size_t ---> partition number, vector---> centers of a partition
	std::vector<unsigned int>partitionHigh; //high value of each partition.
	std::vector<std::map <size_t, std::string> >decoded_center_in_partition;//size_t--->center index, string --->dna sequence
