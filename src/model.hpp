...
class initial_alignment_set {
	public:
	initial_alignment_set(const all_data & d, const T & model, double base_cost): data(d), model(model), sorted_original_als(d.numAlignments()) {
	...
	initial_alignment_set(const all_data & d, const set< const pw_alignment *, compare_pw_alignment> & als, const T & model, double base_cost): data(d), model(model), sorted_original_als(als.size()) {
		this->base_cost = base_cost;
		multimap<double, const pw_alignment*> sorter;
		double sumgain = 0;
