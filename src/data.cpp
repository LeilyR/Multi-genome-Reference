...
void all_data::read_accknown_fasta_sam(std::string fasta_all_sequences, std::string sam_all_alignments){
	std::ifstream fastain(fasta_all_sequences.c_str());
	if(fastain) {
		std::string str;
		std::stringstream curseq;
	...
	//Read the alignments from sam file
	std::cout << "readAlignment sam file"<<std::endl;
	...
	while(samIn.ReadRecord(samHeader, samRecord)){
		...
		// Achtung : sam file considere the file reference as an uniqe sequence even if there are different sequences on the reference, so the positions are concatenated.
		//e.g. if I take the first base of the second sequence from my reference.fasta, it will not return 1 but lengthOfTheSequence1 + 1 !
		...
		for(int index = 0; index < samRecord.getReadLength(); index++)
		{
			...
			else
			{
				...
				{
					alReadSeq += '-';
					alRefSeq += myRefSeq[samRecord.get1BasedPosition() + prevRef];

				}
				//now we are at a spot in both the reference and the read, so add it.
				alReadSeq += currentSeq[index];
				alRefSeq += myRefSeq[samRecord.get1BasedPosition() + refOffset];
			}
		}
		...
		std::string name1;
		name_split(samRecord.getReferenceName(), acc1, name1);
		size_t start1 = samRecord.get1BasedPosition() - 1 ; //XXX why -1?
		size_t incl_end1 = start1 + tmpCigar->getExpectedReferenceBaseCount() - 1 ;
		std::string tmpString = acc1 + ":" + samRecord.getReferenceName();
	//	std::cout << "tmpString1 " << tmpString << std::endl;
		...
		// In Sam file : Reverse and Forward ??
		std::string tmpString2 = acc2 + ":" + samRecord.getReadName();
	//	std::cout << "tmpString2 " << tmpString << std::endl;
		std::map<std::string, size_t>::iterator findseq2 = longname2seqidx.find((samRecord.getReadName()));
		if(findseq2==longname2seqidx.end()) {
			std::cerr << "Error: unknown sequence in sam file : " << samRecord.getReadName() << std::endl;
			exit(1);
		}
		...
		if(idx1 != idx2 || !(start1==start2 && incl_end1 == incl_end2)) {
			...
			if(rev_skip == false){
				...
				for(size_t i =0; i < alignments.size();i++){ 
					...
					if(al.getreference2() == idx1 && al.getreference1()==idx2 && al.getbegin2()==start1 && al.getbegin1()==start2 && al.getend2()== incl_end1 && al.getend1()==incl_end2){
						skip = true;
						break;
					}
				}
		...
		while(getline(mafin, str)) {
			if(str.at(0)!='#') { // skip headers
				if(str.at(0)=='a') { // next alignment
					...
					//Skip if two references are reverse of eachother
				//	if(idx1==idx2 && start1 == incl_end2 && start2 == incl_end1){
				//		skip_self++;
				//	}
					// both al parts not identical	
					if(idx1 != idx2 || !(start1==start2 && incl_end1 == incl_end2)) {//TODO : if idx1 != idx2 else: if(!(start1==start2 && incl_end1 == incl_end2)||!(start1==incl_end2 && start2 == incl_end1)
						bool rev_skip = false;
					//	for(size_t i = 0; i < alignments.size();i++){
					...
					//	}
						if(rev_skip == false){
							bool skip = false;
							// check if we already have an alignment with same coordinates
							std::set<pw_alignment, compare_pw_alignment>::iterator find_al = setOfAls.find(al);
							if(find_al!=setOfAls.end()){
								skip = true;
							}
						/*	for(size_t i =0; i < alignments.size();i++){
...
//	for(size_t i = 0; i < alignments.size();i++){
//		pw_alignment p = alignments.at(i);
//		if(p.getreference1()==p.getreference2() && p.getbegin1()==p.getend2()&&p.getbegin2()==p.getend1()){
//			p.print();
//		}
//		else{
		...
		while(getline(fastain, str)) {
			if(str.at(0)=='>') {
				if(0!=curname.compare("")) { // store previous sequence
					if(curseq.str().size()!=0){
						insert_sequence(curacc, curname, curseq.str());
					//	if(stoi(curname) == 12 && curacc == "graph5"){
					//		std::ofstream out("cluster12");
					//		std::cout <<"node 24 from 35659 to 36799 "<<std::endl;
					//		out << curseq.str();
					//		for(size_t i = 35659; i<= 36799;i++){
					//			std::cout << curseq.str().at(i);
					//		}
					//		std::cout << " "<<std::endl;
					//	}
			...
			if(str.at(0)!='#') { // skip headers
				...
				//	}
					...
					if(idx1 != idx2 || !(start1==start2 && incl_end1 == incl_end2)) {//TODO : if idx1 != idx2 else: if(!(start1==start2 && incl_end1 == incl_end2)||!(start1==incl_end2 && start2 == incl_end1)
						bool rev_skip = false;
					//	for(size_t i = 0; i < alignments.size();i++){
						std::cout <<  start1<< " "<< start2 << " "<< incl_end1<< " "<< incl_end2<<std::endl;
						pw_alignment al(al1, al2, start1, start2, incl_end1, incl_end2, idx1, idx2);
						//Check if the reverse of the alignment exists
						pw_alignment p;
						al.get_reverse(p);
						...
						/*	for(size_t i =0; i < alignments.size();i++){
							...
							//	pw_alignment al(al1, al2, start1, start2, incl_end1, incl_end2, idx1, idx2);
								if((idx1 == idx2)&&(start1 == incl_end2)&&(start2==incl_end1)){
									...
									//	p1.print();
									//	exit(1);
										alignments.push_back(p1);
										setOfAls.insert(p1);
										located_alignment sal(this, p1);
										l_alignments.push_back(sal);
...
//}
	...
	int all_data::findIdAlignment(std::string name,unsigned int start, unsigned int end){ // declare in graph or data ?
...
//			std::cout << "seq " << i <<" has length 0!"<<std::endl;
//		}
//	}
//	std::cout << "total base " << base <<std::endl;
	...
	//	}
		...
		uint32_t *temp = reinterpret_cast<uint32_t*> (c1);
		n = *temp;
...
void all_data::compare_seq_with_read(std::ifstream& in, size_t seq_id, size_t & from, size_t & to){
        char c;
        size_t n = 0;
        char c1[sizeof(uint32_t)];
        in.read(c1,sizeof(uint32_t));
        ...
        dnastring sequence = sequences.at(seq_id);
        for(size_t j = from ; j < to; j++){
                std::cout<< "at "<< j;
                c= in.get();
                std::cout<< " : "<< c << " "<< sequence.at(j) <<std::endl;
                assert(sequence.at(j)==c);
        }

}

void all_data::checkAlignmentRange(const pw_alignment & al)const{
	size_t ref1 = al.getreference1();
	assert(ref1 >= 0 && ref1 < sequences.size());
	size_t ref2 = al.getreference2();
	assert(ref2 >=0 && ref2 < sequences.size());
	size_t l1,r1,l2,r2;
	al.get_lr1(l1,r1);
	al.get_lr2(l2,r2);
	assert(l1 < sequences.at(ref1).length() && r1 < sequences.at(ref1).length());//it checks if both left and right are smaller than length of the sequence
	assert(l2 < sequences.at(ref2).length()&& r2 < sequences.at(ref2).length());
	// check if it is part of an existing alignment:
	...
	for(size_t i =0; i < alignments.size();i++){
		...
		if(p.getreference1() == ref1 && l1 >= left1 && r1 <= right1){
			if(p.getreference2() == ref2 && l2 >= left2 && r2 <= right2){
				boundaryExists = true;
				break;
			}
		}
	}
	if(boundaryExists==false){
		exit(1);
	}
}
...
std::string all_data::extract_seq_part(unsigned int & ref , size_t & from, size_t & to)const{
	std::string this_part;
	for(size_t i =from ; i <= to; i++){
		this_part+=sequences.at(ref).at(i);
	}
	return this_part;
}

std::string all_data::extract_reverse_seq_part(unsigned int & ref , size_t & from, size_t & to)const{
	std::string this_part;
	for(size_t i = to ; i > from ; i--){
		this_part+=dnastring::complement(sequences.at(ref).at(i));
	}
	this_part+=dnastring::complement(sequences.at(ref).at(from));
	return this_part;
}
...
void overlap::test_no_overlap_between_ccs(const pw_alignment & p, std::set<const pw_alignment*, compare_pointer_pw_alignment> & ccs)const{
...
//void mc_model::markov_chain_alignment(std::ofstream& outs){
	...
	for(size_t k = 0; k < data.numAlignments(); k++){
		...
		computing_modification_twoToOne(p,acc2,functor);
	}
	...
	for(size_t i = 0; i< data.numAcc();i++){
		for(size_t j = 0; j < data.numAcc();j++){
			...
			//	unsigned int total = 0;
				size_t bit = 12; // number of bits to use for encoding event width //TODO
...
//	std::cout<<"length: "<< length<<std::endl;
	...
	computing_modification_twoToOne(p,acc2,f);
	std::vector<double> cost_on_sample(2,0);
	std::vector<double> modify_cost(2,0);
	char s1chr;
	char s2chr;
...
//		std::ifstream in("encode", std::ifstream::binary);
	...
	void mc_model::set_alignment_pattern(std::ifstream & in){
		...
		//		std::cout<< "counter: "<< counter<<std::endl;
				it -> second.at(NUM_KEEP+NUM_DELETE+9)=powersOfTwo.at(bit); //TODO
...
//			std::cout<<"i+n: "<< i+n<<std::endl;
	...
	void mc_model::computing_modification_twoToOne(const pw_alignment & p, size_t & acc2, abstract_context_functor & functor)const{//I needed to add acc2 as an argument because otherwise couldn't encode the long centers
		std::string seq = "";
		size_t acc1 = data.accNumber(p.getreference1());
	//	size_t acc2 = data.accNumber(p.getreference2());
		size_t first_patterns = Alignment_level;
...
//		}
	...
	void mc_model ::get_encoded_member(const pw_alignment & al,size_t center_ref,size_t center_left, encoding_functor & functor,std::ofstream& outs)const{
		...
		if(al.getreference1()==center_ref && left_1 == center_left){
			...
			computing_modification_twoToOne(al, acc2 ,functor);
		}
	}
	void mc_model ::get_encoded_member_long_center(const pw_alignment & al,unsigned int & center_ref,encoding_functor & functor,std::ofstream& outs)const{
	//	size_t acc1 = data.accNumber(al.getreference1());
		size_t accession = data.accNumber(center_ref);
	//	size_t left_1; 
		size_t left_2;
	//	size_t right_1;
		size_t right_2;
		std::cout<< "long al length: "<< al.alignment_length()<<std::endl;
	//	al.get_lr1(left_1,right_1);
		al.get_lr2(left_2,right_2);
	...
	//	std::string pattern;
		...
		size_t last_base = dnastring::base_to_index(last_center_base);
		final_pattern += last_base;
//		std::cout<<"final pattern "<<std::endl;
		for(size_t i =0; i < final_pattern.size();i++){
			std::cout << int(final_pattern.at(i))<< " ";
		}
		std::cout << " " << std::endl;
		...
		assert(it!= highValue.at(acc1).at(acc2).end());
...
*/   	
	...
	//		std::cout << "low values " << low.at(h) << " high values: " << high.at(h)<<std::endl;
	//	}
	...
	//	std::cout<< "center acc should be: " << acc1 << std::endl;
		if(acc2 == 0 && p.getreference1()==3 && p.getbegin1()==121543){
