...
template<typename alignment>
bool compare_alignment<alignment>::operator()(const alignment_type &ar, const alignment_type &br) const {
// if(ar.getbegin(0)
	const alignment_type * a = &ar;
	const alignment_type * b = &br;
	size_t asmaller = 0;//TODO replace it with calling the new one
	size_t abigger = 1;
	if(a->getreference(0) > a->getreference(1)) {
		asmaller = 1;
...
//              al_encode << " low: "<< low << " high: "<< high <<std::endl;
        ...
        void wrapper::context(int context, std::ofstream & encode_context){
	//	encode_context.write(reinterpret_cast<char*>(&context), sizeof(size_t));

		encode_context<<"c"<<context;
